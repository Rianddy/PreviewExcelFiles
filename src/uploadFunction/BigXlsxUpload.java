 ...
 *         can preview huge xlsx excel file's data within limited memory. And then 
 *         send those data back with JSON format.
 * 
 * @author Rianddy
 * 
 */
public class BigXlsxUpload {
		...
		public void endElement(String uri, String localName, String name)
			...
			if (maxRowPreview < 21) {
				...
				if ("v".equals(name)) {
					...
					if (twentyRows == 1) {
						columnType[sheetIndex][currentColumn] = typeName;

					}

					if (twentyRows > 1 && same) {
						if (columnType[sheetIndex][currentColumn] != typeName) { // output.print(currentColumn);
							columnType[sheetIndex][currentColumn] = "string";
							same = false;
						}

						// else
						// columnType[currentColumn]=String.valueOf(nextDataType);

						// output.print(thisColumn);
						// output.print(',');
						// output.print(columnType[lastColumnNumber]);
					}
					// output.print(',');
					// output.print(columnType[currentColumn]);
					currentColumn += 1;
					// Update column
					if (thisColumn > -1)
					...
					currentColumn = 0;
					twentyRows += 1;
					if (thisStr == null && thisColumn == 1)
						previewJson[sheetIndex] += "";
					else
						previewJson[sheetIndex] += "\"],";
					maxRowPreview++;
					output.println(maxRowPreview);
					// output.print("row");
					// output.print(twentyRows);
					// output.print(':');
				}
			}
		}
	...
	public BigXlsxUpload(OPCPackage pkg, PrintStream output, int minColumns) {
	...
	public String GetColumnType(int sheetIndex) {
		...
		for (i = 0; i < (columnType[sheetIndex].length - 1); i++) {
			if (columnType[sheetIndex][i] == null)
				...
				if (columnType[sheetIndex][i].equals("SSTINDEX"))
					columnType[sheetIndex][i] = "string";
